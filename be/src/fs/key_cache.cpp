...
static const std::string& get_identifier_from_pb(const EncryptionKeyPB& pb) {
    ...
    case VAULT_PLAIN_KEY:
        // assume single master/global vault key for now, so use a fixed identifier,
        // to prevent key desc change causing all data not accessible
        return VAULT_KEY_IDENTIFIER;
    case KMS_KEY:
        return pb.key_desc();
    case KMS_DATA_KEY:
        return pb.key_desc();
