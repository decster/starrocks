...
public:
    virtual Status encode_chunk_to_full_row_column(const Schema& schema, const Chunk& chunk, BinaryColumn* dest_column);
    virtual Status encode_columns_to_full_row_column(const Schema& schema, const std::vector<Column*>& columns,
                                                     BinaryColumn& dest);
    ...
    virtual void combine_key_with_ver(std::string& key, const int8_t op, const int64_t version);
    virtual Status split_key_with_ver(const std::string& ckey, std::string& key, int8_t& op, int64_t& version);

protected:
    void prepare_ops_datas_selective(const Schema& schema, int from_col, int to_col,
                                             std::vector<const void*>* pdatas, const Chunk& chunk,
                                             std::vector<EncodeOp>* pops);

    void prepare_ops_datas_selective_for_columns(const Schema& schema, int from_col, int ncol,
                                                                 std::vector<const void*>* pdatas,
                                                                 const std::vector<Column*>& columns,
                                                                 std::vector<EncodeOp>* pops);

private:
    virtual void variable_slice_encode(std::vector<EncodeOp>* pops, int col_id, bool is_last = false);
    virtual Status variable_slice_decode(Slice* src, std::string* dest, bool is_last = false);
};
